...
async def create_embassy(embassy_in):
    # emb_name = await Embassy.find_one({"name": embassy_in.name})
    # if emb_name is not None:
    #     raise HTTPException(
    #         status.HTTP_409_CONFLICT, detail="user with that email already exists"
    #     )

    #     # Convert EmbassyIn to Embassy
    # embassy = embassy_in.model_dump_json()
    return ""
    # return await embassy.create(embassy)
